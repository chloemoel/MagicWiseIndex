...
process CALCULATE_BEWISE_ARRAY {
    ...
    sample_names = m_values.columns[1:]
    probe_info.dropna(inplace=True)
    score = m_values.merge(probe_info, left_on=m_values.columns[0], right_on="ID")
    ...
    ## output csv with samples as rows, rows of samples 
